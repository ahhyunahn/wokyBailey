...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        state = [] 
        state["startPosition"] = self.startingPosition
        state["cornersToVisit"] = self.corners
        state["currentPosition"] = self.startingPosition

        # check if the starting position is corner
        if state["startPosition"] in state["cornersToVisit"]:
            state["cornersToVisit"].remove(state["startPosition"])
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successorState["currentPosition"] = (nextx, nexty)

                # check if successorState's position is a corner, update the 
                # "cornersToVisit" (visited corners log)
                if successorState["currentPosition"] in successorState["cornersToVisit"]:
                    successorState["cornersToVisit"].remove(successorState["currentPosition"])
